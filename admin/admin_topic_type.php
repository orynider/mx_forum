...
while ( $row = $db->sql_fetchrow( $result ) )
{
	...
	{
		$tt_image = $module_root_path . TEMPLATE_ROOT_PATH . 'images/topic_icons/' . $row['topic_type_image'] ;
	}
	else
	{
		$tt_image = $phpbb_root_path . 'templates/subSilver/images/folder_topic_type.gif';
	}
	...
	$template->assign_block_vars( 'topic_loop', array( 'TOPIC_TYPE' => stripslashes( trim( $row['topic_type_name'] ) ),
...
$sql = "SELECT t1.topic_type_color AS announce_color, t2.topic_type_color AS sticky_color
  FROM " . TOPIC_ADD_TYPE_TABLE . " t1, " . TOPIC_ADD_TYPE_TABLE . " t2
...
if ( isset( $HTTP_POST_VARS['submit_announce_color'] ) || isset( $HTTP_POST_VARS['submit_sticky_color'] ) )
{
...
else if ( isset( $HTTP_POST_VARS['add_topic_type'] ) )
{
	...
	$topic_icon = post_icons( 'topic_icons/', stripslashes( $first['topic_type_image'] ), 'modules/mx_forum/' );

	$template->set_filenames( array( "body" => "admin/admin_edit_topic_type.tpl" ) 
		);

	$template->assign_vars( array( 'L_EDIT_TOPIC_TYPE' => $lang['configuration'],
...
if ( isset( $HTTP_POST_VARS['submit'] ) )
{
	switch ( $mode )
	{
        ...
        SET topic_type_name = '" . htmlspecialchars( addslashes( trim( $HTTP_POST_VARS['tt_name'] ) ) ) . "', topic_type_color = '" . ( ( !empty( $HTTP_POST_VARS['tt_color'] ) ) ? ( trim( $HTTP_POST_VARS['tt_color'] ) ) : '000000' ) . "', topic_type_auth = '" . intval( $HTTP_POST_VARS['topic_type_auth'] ) . "', topic_type_active = '" . $HTTP_POST_VARS['tt_active'] . "', topic_type_image = '" . trim( addslashes( $HTTP_POST_VARS['menuicons'] ) ) . "', topic_type_image_new = '" . trim( addslashes( $HTTP_POST_VARS['tt_folder_new'] ) ) . "'
        WHERE topic_type_id = $id";
        ...
        FROM " . TOPIC_ADD_TYPE_TABLE ;
          ...
          VALUES ( '" . htmlspecialchars( addslashes( trim( $HTTP_POST_VARS['tt_name'] ) ) ) . "', '" . intval( $row_max['max_id'] + 1 ) . "', '" . intval( $HTTP_POST_VARS['topic_type_auth'] ) . "', '" . $HTTP_POST_VARS['tt_active'] . "', '" . intval( $row_max['max_order'] + 1 ) . "', '" . ( ( !empty( $HTTP_POST_VARS['tt_color'] ) ) ? ( trim( $HTTP_POST_VARS['tt_color'] ) ) : '000000' ) . "', '" . trim( addslashes( $HTTP_POST_VARS['menuicons'] ) ) . "', '" . trim( addslashes( $HTTP_POST_VARS['tt_folder_new'] ) ) . "')";
