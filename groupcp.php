 ...
 *
 *   $Id: groupcp.php,v 1.4 2005/09/15 17:59:19 jonohlsson Exp $
 *
 *
 ***************************************************************************/
...
{
	global $lang, $images, $board_config, $phpEx, $mx_forum;
	...
	{
		$email_uri = ( $board_config['board_email_form'] ) ? $mx_forum->append_sid("profile.$phpEx?mode=email&amp;" . POST_USERS_URL .'=' . $row['user_id']) : 'mailto:' . $row['user_email'];
	...
	$temp_url = $mx_forum->append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $row['user_id']);
	...
	$temp_url = $mx_forum->append_sid("privmsg.$phpEx?mode=post&amp;" . POST_USERS_URL . "=" . $row['user_id']);
	...
	$www_img = ( $row['user_website'] ) ? '<a href="' . $row['user_website'] . '" target="_userwww"><img src="' . $mx_forum->images['icon_www'] . '" alt="' . $lang['Visit_website'] . '" title="' . $lang['Visit_website'] . '" border="0" /></a>' : '';
	$www = ( $row['user_website'] ) ? '<a href="' . $row['user_website'] . '" target="_userwww">' . $lang['Visit_website'] . '</a>' : '';

	if ( !empty($row['user_icq']) )
	{
		$icq_status_img = '<a href="http://wwp.icq.com/' . $row['user_icq'] . '#pager"><img src="http://web.icq.com/whitepages/online?icq=' . $row['user_icq'] . '&img=5" width="18" height="18" border="0" /></a>';
		$icq_img = '<a href="http://wwp.icq.com/scripts/search.dll?to=' . $row['user_icq'] . '"><img src="' . $mx_forum->images['icon_icq'] . '" alt="' . $lang['ICQ'] . '" title="' . $lang['ICQ'] . '" border="0" /></a>';
	...
	$aim_img = ( $row['user_aim'] ) ? '<a href="aim:goim?screenname=' . $row['user_aim'] . '&amp;message=Hello+Are+you+there?"><img src="' . $mx_forum->images['icon_aim'] . '" alt="' . $lang['AIM'] . '" title="' . $lang['AIM'] . '" border="0" /></a>' : '';
	$aim = ( $row['user_aim'] ) ? '<a href="aim:goim?screenname=' . $row['user_aim'] . '&amp;message=Hello+Are+you+there?">' . $lang['AIM'] . '</a>' : '';

	$temp_url = $mx_forum->append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=" . $row['user_id']);
	$msn_img = ( $row['user_msnm'] ) ? '<a href="' . $temp_url . '"><img src="' . $mx_forum->images['icon_msnm'] . '" alt="' . $lang['MSNM'] . '" title="' . $lang['MSNM'] . '" border="0" /></a>' : '';
	$msn = ( $row['user_msnm'] ) ? '<a href="' . $temp_url . '">' . $lang['MSNM'] . '</a>' : '';

	$yim_img = ( $row['user_yim'] ) ? '<a href="http://edit.yahoo.com/config/send_webmesg?.target=' . $row['user_yim'] . '&amp;.src=pg"><img src="' . $mx_forum->images['icon_yim'] . '" alt="' . $lang['YIM'] . '" title="' . $lang['YIM'] . '" border="0" /></a>' : '';
	$yim = ( $row['user_yim'] ) ? '<a href="http://edit.yahoo.com/config/send_webmesg?.target=' . $row['user_yim'] . '&amp;.src=pg">' . $lang['YIM'] . '</a>' : '';

	$temp_url = $mx_forum->append_sid("search.$phpEx?search_author=" . urlencode($username) . "&amp;showresults=posts");
...
if ( isset($HTTP_POST_VARS['groupstatus']) && $group_id )
{
	...
	{
		$mx_forum->redirect($mx_forum->append_sid("login.$phpEx?redirect=groupcp.$phpEx&" . POST_GROUPS_URL . "=$group_id", true));
	}
	...
	if ( $row['group_moderator'] != $userdata['user_id'] && $userdata['user_level'] != ADMIN )
	{
		...
		$message = $lang['Not_group_moderator'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . $mx_forum->append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . $mx_forum->append_sid("index.$phpEx") . '">', '</a>');
	...
	$template->assign_vars(array(
		'META' => '<meta http-equiv="refresh" content="3;url=' . $mx_forum->append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">')
	);

	$message = $lang['Group_type_updated'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . $mx_forum->append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . $mx_forum->append_sid("index.$phpEx") . '">', '</a>');
...
else if ( isset($HTTP_POST_VARS['joingroup']) && $group_id )
{
	...
	if (	$row = $db->sql_fetchrow($result) )
	{
		if ( $row['group_type'] == GROUP_OPEN )
		{
			do
			{
				if ( $userdata['user_id'] == $row['user_id'] )
				{
					...
					$message = $lang['Already_member_group'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . $mx_forum->append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . $mx_forum->append_sid("index.$phpEx") . '">', '</a>');
		...
		else
		{
			...
			$message = $lang['This_closed_group'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . $mx_forum->append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . $mx_forum->append_sid("index.$phpEx") . '">', '</a>');
	...
	$message = $lang['Group_joined'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . $mx_forum->append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . $mx_forum->append_sid("index.$phpEx") . '">', '</a>');
...
else if ( isset($HTTP_POST_VARS['unsub']) || isset($HTTP_POST_VARS['unsubpending']) && $group_id )
{
	...
	if ( $cancel )
	{
		$mx_forum->redirect($mx_forum->append_sid("groupcp.$phpEx", true));
	}
	...
	if ( $confirm )
	{
		...
		$message = $lang['Unsub_success'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . $mx_forum->append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . $mx_forum->append_sid("index.$phpEx") . '">', '</a>');
	...
	else
	{
		...
		//+mx_forum //include($phpbb_root_path . 'includes/page_header.'.$phpEx); //-mx_forum
		...
		$template->assign_vars(array(
			...
			'L_YES' => $lang['Yes'],
			'L_NO' => $lang['No'],
			'S_CONFIRM_ACTION' => $mx_forum->append_sid("groupcp.$phpEx"),
...
else if ( $group_id )
{
	...
	if ( $group_info = $db->sql_fetchrow($result) )
	{
		...
		if ( !empty($HTTP_POST_VARS['add']) || !empty($HTTP_POST_VARS['remove']) || isset($HTTP_POST_VARS['approve']) || isset($HTTP_POST_VARS['deny']) )
		{
			...
			if ( !$is_moderator )
			{
				...
				$message = $lang['Not_group_moderator'] . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . $mx_forum->append_sid("index.$phpEx") . '">', '</a>');
			...
			if ( isset($HTTP_POST_VARS['add']) )
			{
				...
				if ( !($row = $db->sql_fetchrow($result)) )
				{
					...
					$message = $lang['Could_not_add_user'] . "<br /><br />" . sprintf($lang['Click_return_group'], "<a href=\"" . $mx_forum->append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_index'], "<a href=\"" . $mx_forum->append_sid("index.$phpEx") . "\">", "</a>");
				...
				if ( $row['user_id'] == ANONYMOUS )
				{
					...
					$message = $lang['Could_not_anon_user'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . $mx_forum->append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . $mx_forum->append_sid("index.$phpEx") . '">', '</a>');
				...
				else
				{
					...
					$message = $lang['User_is_member_group'] . '<br /><br />' . sprintf($lang['Click_return_group'], '<a href="' . $mx_forum->append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . $mx_forum->append_sid("index.$phpEx") . '">', '</a>');
	...
	// Generate the fold/unfold categories switches---------------------------------group_info
	if ( ($HTTP_COOKIE_VARS['phpbbCategory_group_info']) != '')
	{
		$cat_on = $HTTP_COOKIE_VARS['phpbbCategory_group_info'];
	}
	else
	{
		$cat_on = false;
    ...
    if( $cat_on )
    {
     	$template->assign_block_vars('switch_cat_on', array());
    }
   	else
    {
     	$template->assign_block_vars('switch_cat_off', array());
    }	
    
	// Generate the fold/unfold categories switches--------------------------------member_info
	if ( ($HTTP_COOKIE_VARS['phpbbCategory_member_info']) != '')
	{
		$cat_on = $HTTP_COOKIE_VARS['phpbbCategory_member_info'];
	}
	else
	{
		$cat_on = true;
    ...
    if( $cat_on )
    {
     	$template->assign_block_vars('switch_catt_on', array());
    }
   	else
    {
     	$template->assign_block_vars('switch_catt_off', array());
    }	    
	
    $cat_nav_icon_url = PORTAL_URL . TEMPLATE_ROOT_PATH . 'images/';
    
	$template->assign_vars(array(
		"U_CAT_NAV"  => $cat_nav_icon_url,
		'L_GROUP_HOME' => $lang['Group_Home'],
		...
		'U_MOD_VIEWPROFILE' => $mx_forum->append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$user_id"), 
		'U_SEARCH_USER' => $mx_forum->append_sid("search.$phpEx?mode=searchuser"), 
		...
		'S_GROUPCP_ACTION' => append_sid("index.$phpEx?page=" . $mx_forum->forum_pages['groupcp'] . "&" . POST_GROUPS_URL . "=$group_id"),
		'U_GROUP_HOME' => append_sid("index.$phpEx?page=".$mx_forum->forum_pages['groupcp'])
		)
	);
	...
	for($i = $start; $i < min($board_config['topics_per_page'] + $start, $members_count); $i++)
	{
		...
		if ( $group_info['group_type'] != GROUP_HIDDEN || $is_group_member || $is_moderator )
		{
			...
			$template->assign_block_vars('member_row', array(
				...
				'U_VIEWPROFILE' => $mx_forum->append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$user_id"))
			);
	...
	$template->assign_vars(array(
		'PAGINATION' => $mx_forum->generate_pagination("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id", $members_count, $board_config['topics_per_page'], $start),
...
/*
	...
	if ( $s_group_list_opt != '' || $s_pending_groups_opt != '' || $s_member_groups_opt != '' )
	{
		...
		$template->assign_vars(array(
			...
			'S_USERGROUP_ACTION' => $mx_forum->append_sid("groupcp.$phpEx"), 
			'S_HIDDEN_FIELDS' => $s_hidden_fields, 
...
$funktioner_nu = array('Samskolans personal', 'Skolledning', 'Expedition', 'Stora skolans Elevråd', 'Stora skolans Webbtidning', 'Sajtkontrollanter' );

$klasser_nu = array('Stora2004', 'Stora2003', 'Stora2002');
$gymnasieklasser_nu = array('Gymnasiet2004', 'Gymnasiet2003', 'Gymnasiet2002');

$klasser_arkiv = array('Stora2001', 'Stora2000', 'Stora1999');
$gymnasieklasser_arkiv = array('Gymnasiet2001', 'Gymnasiet2000', 'Gymnasiet1999');

$template->set_filenames(array(
			'user' => 'groupcp_user_body_sam.tpl')
		);
...
for($arskurs = 0; $arskurs < count($klasser_nu); $arskurs++) 
{

$template->assign_block_vars("level.arskurs", array());
	
$sql = "SELECT group_id, group_name, group_description, group_type 
		FROM " . GROUPS_TABLE . " g 
		WHERE group_single_user <> " . TRUE . " 
			AND group_description = '".$klasser_nu[$arskurs]."' 
		ORDER BY g.group_name";
	if ( !($result = $db->sql_query($sql)) )
	{
	...
	while( $k_row = $db->sql_fetchrow($result) )
	{
		$klass_link = append_sid( PORTAL_URL . 'index.'. $phpEx.'?page=' . $mx_forum->forum_pages['groupcp']. '&g=' .$k_row['group_id'] );
		
		$template->assign_block_vars("level.arskurs.klasser", array(
          "KLASSNAMN" => $k_row['group_name'],
          "KLASSLINK" => $klass_link
		));
	}
}
...
// ------------------------------------------------------------------------------Gymnasiet nu
$template->assign_block_vars("level", array(
	"TITLE" => 'Gymnasieklasser'
));

for($arskurs = 0; $arskurs < count($gymnasieklasser_nu); $arskurs++) 
{
...
$sql = "SELECT group_id, group_name, group_description, group_type 
		FROM " . GROUPS_TABLE . " g 
		WHERE group_single_user <> " . TRUE . " 
			AND group_description = '".$gymnasieklasser_nu[$arskurs]."' 
		ORDER BY g.group_name";
	if ( !($result = $db->sql_query($sql)) )
	{
		mx_message_die(GENERAL_ERROR, 'Error getting group information', '', __LINE__, __FILE__, $sql);
...
// ------------------------------------------------------------------------------Funktioner
$template->assign_block_vars("funktion", array(
	"TITLE" => 'Övriga'
));

for($arskurs = 0; $arskurs < count($funktioner_nu); $arskurs++) 
{
	if ( $arskurs == 0 || $arskurs == 3 || $arskurs == 6)
	{
		$template->assign_block_vars("funktion.rad", array(
		));
	}
	...
	$sql = "SELECT group_id, group_name, group_description, group_type 
		FROM " . GROUPS_TABLE . " g 
		WHERE group_single_user <> " . TRUE . " 
			AND group_description = '".$funktioner_nu[$arskurs]."' 
		ORDER BY g.group_name";
	if ( !($result = $db->sql_query($sql)) )
	{
		mx_message_die(GENERAL_ERROR, 'Error getting group information', '', __LINE__, __FILE__, $sql);
	...
	while( $k_row = $db->sql_fetchrow($result) )
	{
		...
		$template->assign_block_vars("funktion.rad.id", array(
          "KLASSNAMN" => $k_row['group_name'],
          "KLASSLINK" => $klass_link
		));
	}
...
$template->assign_block_vars("level_arkiv", array(
	"TITLE" => 'År 7-9 (arkiv)',
	"NUM" => '1',
	"U_CAT_NAV" => $cat_nav_icon_url
));

	if ( ($HTTP_COOKIE_VARS['phpbbCategory_1']) != '')
	{
		$cat_on = $HTTP_COOKIE_VARS['phpbbCategory_1'];
	}
	else
	{
		$cat_on = false;
    ...
    if( $cat_on )
    {
     	$template->assign_block_vars('level_arkiv.switch_cat_on', array());
    }
   	else
    {
     	$template->assign_block_vars('level_arkiv.switch_cat_off', array());
    }	
    
for($arskurs = 0; $arskurs < count($klasser_arkiv); $arskurs++) 
{

$ex_year = intval( str_replace("Stora", "",$klasser_arkiv[$arskurs]) )	+ 3;

$template->assign_block_vars("level_arkiv.arskurs", array(
	"YEAR" => $ex_year
));
...
$sql = "SELECT group_id, group_name, group_description, group_type 
		FROM " . GROUPS_TABLE . " g 
		WHERE group_single_user <> " . TRUE . " 
			AND group_description = '".$klasser_arkiv[$arskurs]."' 
		ORDER BY g.group_name";
	if ( !($result = $db->sql_query($sql)) )
	{
		mx_message_die(GENERAL_ERROR, 'Error getting group information', '', __LINE__, __FILE__, $sql);
	...
	while( $k_row = $db->sql_fetchrow($result) )
	{
		...
		$template->assign_block_vars("level_arkiv.arskurs.klasser", array(
          "KLASSNAMN" => $k_row['group_name'],
          "KLASSLINK" => $klass_link
		));
	}
...
$template->assign_block_vars("level_arkiv", array(
	"TITLE" => 'Gymnasieklasser (arkiv)',
	"NUM" => '2',
	"U_CAT_NAV" => $cat_nav_icon_url
));

	if ( ($HTTP_COOKIE_VARS['phpbbCategory_2']) != '')
	{
		$cat_on = $HTTP_COOKIE_VARS['phpbbCategory_2'];
	}
	else
	{
		$cat_on = true;
...
for($arskurs = 0; $arskurs < count($gymnasieklasser_arkiv); $arskurs++) 
{

$ex_year = intval( str_replace("Gymnasiet", "",$gymnasieklasser_arkiv[$arskurs]) )	+ 3;
...
$sql = "SELECT group_id, group_name, group_description, group_type 
		FROM " . GROUPS_TABLE . " g 
		WHERE group_single_user <> " . TRUE . " 
			AND group_description = '".$gymnasieklasser_arkiv[$arskurs]."' 
		ORDER BY g.group_name";
	if ( !($result = $db->sql_query($sql)) )
	{
		mx_message_die(GENERAL_ERROR, 'Error getting group information', '', __LINE__, __FILE__, $sql);
