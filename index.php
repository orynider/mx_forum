...
if( $mark_read == 'forums' )
{
	...
	$message = $lang['Forums_marked_read'] . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . $mx_forum->append_sid("index.$phpEx?f=".$HTTP_GET_VARS['f']) . '">', '</a> ');
...
if( ( $total_categories = count($category_rows) ) )
{
	...
	for($i = 0; $i < $total_categories; $i++)
	{
		...
		{
			if ( $is_auth_ary[$forum_data[$j]['forum_id']]['auth_view'] && $forum_data[$j]['cat_id'] == $cat_id && phpbb2_auth_cat($forum_data[$j]['forum_id']))
			{
				//+mxBB_module
